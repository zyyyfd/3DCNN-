...
def cif_to_grid(args):
    ...
    grid_density=args.path_grid_density
    ...
    for i in range(0,len(file_list)):
        ...
        grid_coords_original=np.hstack((grid_coords_original.reshape(-1,1),y_original1.reshape(-1,1)))
        grid_coords_original=np.hstack((grid_coords_original,z_original1.reshape(-1,1)))
        grid_coords_original=grid_coords_original.astype(np.float32)

        #将元素符号输入网格
        grid_symbols_original=np.zeros((x_namber,y_namber,z_namber,13),dtype=np.int8)
        ...
        for i in range(0,len(coords)):
            ...
            grid_symbols_original[x_tier_original][y_tier_original][z_tier_original]+=symbols_nambers[i]
...
def resize_data(args):
    ...
    for i in range(0,len(file_list)):
        ...
        zoom_factors = (new_shape[0]/original_shape[0], 
                        ...
                        new_shape[2]/original_shape[2],1)
        # 将数据分成 num_threads 个子数组
        ...
        for y in range(0,num_threads):
            thread = threading.Thread(target=zoom_subarray, args=(subarrays[y],zoom_factors, result, y))
            thread.start()
...
def creatlabel0_1(args):
    path_labels=args.path_labels
    path_label=args.path_label
    pd_labels=pd.read_excel(path_labels)
    np_labels= np.array(pd_labels)
    dataset1=np_labels[:,1]
    dataset1=dataset1.reshape(-1,1).astype(np.float32)
    scaler=MinMaxScaler(feature_range = (0,1)).fit(dataset1)
    dataset2=scaler.transform(dataset1)
    dataset2=dataset2.reshape(len(pd_labels),).astype(np.float32)
    np_labels[:,1]=dataset2
